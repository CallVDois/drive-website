...
import type { CreateFolderDTO, CreateFolderResponseDTO, FolderDetailDTO } from "./folder.types";
...
export async function createFolder(request: CreateFolderDTO): Promise<string> {
    try {
        const response = await api.post<CreateFolderResponseDTO>(`/folders`, request)
